...
"""
#
# (C) Pywikipedia bot team, Legoktm, Earwig 2005-2013
# Slightly modified by Legoktm to remove pywikipedia dependencies
# Code added in to test against mwparserfromhell
#
...
log('Starting run.')
log('Using ' + ('C' if mwparserfromhell.parser.use_c else 'Python') + ' parser.')

#do shit
dump = XmlDump(path)
gen = dump.parse()
for page in gen:
    name = u'{0}@{1}'.format(page.title, page.revisionid)
    log(name)
    try:
        parsed = mwparserfromhell.parse(page.text)
        if not parsed.filter_templates():
    ...
    if page.text == parsed:
        log('Roundtripped properly.')
    else:
        log('Did not roundtrip properly.')
        log(name, lf=badfile)
